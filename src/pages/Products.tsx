...
const Products = () => {
  ...
  const handleSave = () => {
    if (!form.name || !form.barcode) { toast({ title: "خطأ", description: "الاسم والباركود مطلوبان", variant: "destructive" }); return; }
    const status = form.stock === 0 ? "نفد" : form.stock <= form.reorderLevel ? "منخفض" : "متوفر";
    if (editing) { updateProduct({ ...form, id: editing.id, status } as Product); toast({ title: "تم التحديث بنجاح" }); }
    else { addProduct({ ...form, status }); toast({ title: "تم إضافة المنتج بنجاح" }); }
    setDialogOpen(false);
  };

  const handleDelete = () => {
    if (deletingId) { deleteProduct(deletingId); toast({ title: "تم حذف المنتج" }); }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{products.length} منتج • {products.filter(p => p.status === "منخفض").length} منخفض المخزون</p>
        </div>
        <Button onClick={openAdd}><Plus className="w-4 h-4 ml-2" />إضافة منتج</Button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="glass-card rounded-xl p-4 flex items-center gap-3">
          ...
          <div><p className="text-xl font-bold text-card-foreground">{products.length}</p><p className="text-xs text-muted-foreground">إجمالي المنتجات</p></div>
        </div>
        <div className="glass-card rounded-xl p-4 flex items-center gap-3">
          ...
          <div><p className="text-xl font-bold text-card-foreground">{products.filter(p => p.status === "منخفض").length}</p><p className="text-xs text-muted-foreground">مخزون منخفض</p></div>
        </div>
        <div className="glass-card rounded-xl p-4 flex items-center gap-3">
          ...
          <div><p className="text-xl font-bold text-card-foreground">{products.filter(p => p.status === "نفد").length}</p><p className="text-xs text-muted-foreground">نفد المخزون</p></div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="بحث بالاسم أو SKU أو الباركود..." value={search} onChange={e => setSearch(e.target.value)}
              ...
              {filtered.map((product, i) => {
                ...
                return (
                  <tr key={product.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors animate-fade-in" style={{ animationDelay: `${i * 50}ms` }}>
                    ...
                    <td className="p-3"><span className={cn("text-[11px] px-2 py-0.5 rounded-full border font-medium", statusVariant(product.status))}>{product.status}</span></td>
                    <td className="p-3">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{editing ? "تعديل بيانات المنتج" : "أدخل بيانات المنتج الجديد"}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div><label className="text-xs text-muted-foreground">اسم المنتج *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs text-muted-foreground">SKU</label><input value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
              <div><label className="text-xs text-muted-foreground">الباركود *</label><input value={form.barcode} onChange={e => setForm({ ...form, barcode: e.target.value })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs text-muted-foreground">سعر الشراء</label><input type="number" value={form.buyPrice || ""} onChange={e => setForm({ ...form, buyPrice: Number(e.target.value) })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
              <div><label className="text-xs text-muted-foreground">سعر البيع</label><input type="number" value={form.sellPrice || ""} onChange={e => setForm({ ...form, sellPrice: Number(e.target.value) })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs text-muted-foreground">الكمية</label><input type="number" value={form.stock || ""} onChange={e => setForm({ ...form, stock: Number(e.target.value) })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
              <div><label className="text-xs text-muted-foreground">حد إعادة الطلب</label><input type="number" value={form.reorderLevel || ""} onChange={e => setForm({ ...form, reorderLevel: Number(e.target.value) })} className="w-full bg-muted border-0 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" /></div>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>تأكيد الحذف</DialogTitle><DialogDescription>هل أنت متأكد من حذف هذا المنتج؟</DialogDescription></DialogHeader>
